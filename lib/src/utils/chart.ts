...
export type BarGroupingMode = 'stacked' | 'side-by-side';
...
export function getEvenDistribution(min: number, max: number, distribution: number) {
    ...
    const remainder = diff % (distribution - 1);
...
export function getYDomain(
    yValues: Array<{
        yValue: Array<{ key: string | number, value: number | undefined | null }>
    }>,
    ...
    const yBounds = getBounds(yValuesFlat);
